...
int state::universalProcess(Grid &g, int x, int y) {
    ...
    if( elem::list[element].evaporates && elemutil::randf() < elem::list[element].evap_chance ) {
        ...
        else g.setPixel(x, y, elem::VOID, true);
        return 1;
    }
    return 0;
}
...
void state::emitterProcess(Grid &g, int x, int y) {
    elem::Elements to_place = elem::emits[ g.getElem(x, y) ];
...
void state::explosionProcess(Grid &g, int x, int y) {
    elem::Elements s = g.getElem(x, y);
    std::string sname = elem::list[ g.getElem(x, y) ].name;
    int size = int(sname[sname.length() - 1] - '0');  // size is the radius
    int radius = size*size;
    int temp = g.getTemp(x, y);
    g.setPixel(x, y, elem::EXPLOSION, true); // normalize to explosion for mutual reactions

    for (int i = -radius; i <= radius; ++i) {
        for (int j = -radius; j <= radius; ++j) {
            if (i*i + j*j <= radius * radius) {
                if( g.inBounds(x+i, y+j) ) {
                    if( g.isEmpty(x+i, y+j) || g.getElem(x+i, y+j) == elem::FIRE)
                        g.setPixel(x + i, y + j, elem::FIRE, true);
                    else {
